...
(function ($) {
    $.fn.listSwitch = function (method) {
        ...
        function _init(target) {
            ...
            // add styles
            $('head').append('<style>.ui-listswitch-buttonbox {width:19px;height:16px;float:left;margin:5em .5em;}.ui-listswitch-buttonbox div{margin-top:.5em}.ui-listswitch-caption{padding: 0 3px;}</style>');

            // buttons
            var buttonBox = $('<div>').addClass(plugin + '-buttonbox');
            var buttons = []
            buttons.push($('<span>').prop('id', targetID + '-addAll').addClass('ui-icon-seek-next'));
            buttons.push($('<span>').prop('id', targetID + '-addOne').addClass('ui-icon-triangle-1-e'));
            buttons.push($('<span>').prop('id', targetID + '-remOne').addClass('ui-icon-triangle-1-w'));
            buttons.push($('<span>').prop('id', targetID + '-remAll').addClass('ui-icon-seek-prev'));
            ...
            $(buttonBox).children('span')
                        .wrap($('<div>').addClass("ui-state-default ui-corner-all"));
            $(buttonBox).children('div')
                        .hover(function () {
            ...
            // left box
            var lBox = $('<select>')
                        .prop({ id: targetID + '_left', multiple: 'multiple' })
                        .css({ width: settings.width, height: settings.height })
                        .addClass('ui-widget-content ui-corner-all');
            if (settings.captions.left) {
                lBox.css('border-top', '0px')
                    .addClass(boxClasses)
            ...
            // right box
            var rBox = $('<select>').prop({ id: targetID + '_right', multiple: 'multiple' })
									.css({ width: settings.width, height: settings.height })
									.addClass('ui-widget-content ui-corner-all');
            if (settings.captions.right) {
            ...
            if (opt.data != null) {
                $.each(opt.data[opt.leftData], function (idx, val) {
                    ...
                    if (opt.data[opt.leftData][idx][opt.titleField]) {
                        o.prop('title', opt.data[opt.leftData][idx][opt.titleField]);
                    }
                    $(lBox).append(o);
                });
            }
            var rBox = $('#' + $(target).prop('id') + '_right');
            rBox.empty();
            if (opt.data != null) {
                $.each(opt.data[opt.rightData], function (idx, val) {
                    ...
                    if (opt.data[opt.rightData][idx][opt.titleField]) {
                        o.prop('title', opt.data[opt.rightData][idx][opt.titleField]);
                    }
                    $(rBox).append(o);
                });
            }
        ...
        function _clearBoxes() {
            $('#' + $(target).prop('id') + ' select').empty();
        };

        function _sortBox(box) {
            var boxlst = $('#' + $(box).prop('id') + ' option');
            boxlst.sort(function (a, b) {
                if (a.text > b.text) return 1;
                else if (a.text < b.text) return -1;
                else return 0;
            });
            $(box).empty().append(boxlst);
        }
        function _getItemList(rightSide) {
            return $('#' + $(target).prop('id') + '_' + (rightSide ? 'right' : 'left') + ' option').get();
        }
        function _setHistory(item, add) {
            var el = $('#' + target.prop('id'))[0];
